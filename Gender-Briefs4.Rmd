...
This briefing showcases the gender landscape in `r country_run` on key indicators helpful for monitoring gender equality and designing effective policy interventions. Gender equality fosters productivity gains, minimizes losses in wealth, reduces poverty, boosts shared prosperity, and supports green, resilient, and inclusive development. `r FCVsentence`
...
\end{minipage}
\vspace{6pt}
`r if(nchar(FCVsentence)>1){paste("\\vspace{16pt}")}`
```
